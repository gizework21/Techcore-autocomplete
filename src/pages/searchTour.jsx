...
const AutocompleteSearch = () => {
  ...
  useEffect(() => {
    async function authenticate() {
      try {
        const token = await login("abebe2@yopmail.com", "Password.1");
        setToken(token);
      } catch (error) {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${BASE_URL}/product/v1/package/auto-complete?q=${encodeURIComponent(
          searchTerm
        )}`,
        {
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="max-w-md w-full bg-white p-4 shadow-lg rounded-lg">
        ...
        <div className="flex flex-col gap-4">
          ...
          <p className="text-[#818083] text-[14px]">
                    ...
                    {destination.tags.map((tag) => (
                      <span
                        ...
                        className={`${getColorForTag(
                          tag.tagId
                        )} text-gray-800 px-2 py-1 rounded`}
                      >
                        {tag.tagName}
                      </span>
                    ))}
